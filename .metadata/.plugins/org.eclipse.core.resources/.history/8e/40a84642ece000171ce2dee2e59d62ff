...
public class TestCheckingAccount {
	public static void main(String[] args) {
		...
		while (input != 4) {
			...
			switch (input) {
				case 1:
					System.out.printf("Your account has $%.2f", CheckingAccount.getBalance());
					break;
				case 2:
					...
					if (amount == 0.0) {
						...
						System.out.printf("You have made a check for $%.2f. Your balance is now $.2f", amount, CheckingAccount.getBalance());
					}
					break;
				case 3:
					...
					while (amount < 0 || amount > CheckingAccount.getBalance()) {
						System.out.printf("Enter a positive number less than $%.2f or zero to cancel:", CheckingAccount.getBalance());
					}
					if (amount == 0.0) {
