...
public class MrKullasPaintStoreRedo {
	...
	public static void main(String[] args) {
		...
		while (!color.equalsIgnoreCase("checkout")) {
			...
			if (color.equalsIgnoreCase("red") || color.equalsIgnoreCase("green")) {
				...
				while (quantity < 0) {// if they don't want anymore, they can enter zero
					System.out.println("You must enter a number sero or greater: ");
					quantity = in.nextInt();
					in.nextLine();
				}
				if (quantity == 0) {
					System.out.println("You did not add any " + color + " paint.");
		...
		System.out.printf("*  Total         %130.2f   *%n", (1.09 * (getRedCost(red) + getGreenCost(green))));
		printThankYou();
		in.close();
	}
