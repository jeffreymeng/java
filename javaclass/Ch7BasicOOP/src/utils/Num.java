...
public class Num {
	 ...
	 */
	//TODO add get digit function. https://stackoverflow.com/questions/19194257/return-the-nth-digit-of-a-number
	//TODO add remove digit function.
	/**
	 ...
	 * @param numbers - Either an array of doubles of any number of arguments containing doubles.
	 * @return an integer, the index of the element containing the greatest double in the array.
	 */
	 ...
	 * @return an integer, the index of the element containing the greatest float in the array.
	 */
	public static double max(float... numbers) {
		int max = 0;
		for (int i = 0; i < numbers.length; i ++) {
			if (numbers[i] > numbers[max]) {
				max = i;
	...
	public static int removeDigits(int start, int end, int number) {
		
		return number % 10^(start-1) + ((number / 10^(end)) * 10^(start-1));
